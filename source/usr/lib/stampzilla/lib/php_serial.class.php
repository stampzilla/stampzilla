...
class phpSerial
{
	...
	*/
	...
	function confBaudRate ($rate)
	{
        ...
        function confStandard ()
        {
            $ret = $this->_exec("stty -F " . $this->_device . " 9600 -parenb cs8 -cstopb clocal -crtscts -ixon -ixoff -echo raw", $out);

            if ($ret !== 0)
            {
                    trigger_error ("Unable to set standard: " . $out[1], E_USER_WARNING);
                    return false;
            }
        }
        function conf ($settings)
        {
            $ret = $this->_exec("stty -F " . $this->_device . " ".$settings, $out);
	...
	function confFlowControl ($mode)
	{
		...
		$linuxModes = array(
			"none"     => "clocal -crtscts -ixon -ixoff -echo raw",
			"rts/cts"  => "-clocal crtscts -ixon -ixoff -echo raw",
